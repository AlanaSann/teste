...
void testQ3() { // 0 é sensivel //1 nao é sensivel
  char str[250];
...
void testQ4() {
  ...
  printf("%d\n", q4(strTexto, strBusca, posicoes) ==
                     2);            // verifica quantas vezes a letra apareceu
  printf("%d\n", posicoes[0] == 5); //+1
  printf("%d\n", posicoes[1] == 8); // ve qual a posiçaõ da primeira e ultiima
                                    // letra impar(fim),par(começo);
  printf("%d\n", posicoes[2] == 34);
  printf("%d\n", posicoes[3] == 37);
  for (i = 0; i < 30; i++) {
    posicoes[i] = -1;
